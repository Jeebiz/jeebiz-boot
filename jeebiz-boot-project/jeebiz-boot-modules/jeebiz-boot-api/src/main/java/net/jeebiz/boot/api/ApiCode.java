...
public enum ApiCode {
	 ...
	 */
	SC_INTERNAL_SERVER_ERROR("500", "error", "服务器内部错误，无法完成请求"),
	/**
	 * 异常：HTTP Status 501（尚未实施）     ->服务器不具备完成请求的功能。 例如，服务器无法识别请求方法时可能会返回此代码。
	 */
	SC_NOT_IMPLEMENTED("501", "error", "服务器不支持请求的功能，无法完成请求"),
	/**
	 * 异常：HTTP Status 502（错误网关）     -> 作为网关或者代理工作的服务器尝试执行请求时，从上游服务器接收到无效的响应。
	 */
	SC_BAD_GATEWAY("502", "error", "错误网关"),
	/**
	 ...
	 */
	SC_SERVICE_UNAVAILABLE("503", "error", "服务器目前无法使用（由于超载或停机维护）"),
	/**
	 * 异常：HTTP Status 504（网关访问超时）     -> 作为网关或者代理工作的服务器尝试执行请求时，未能及时从上游服务器（URI标识出的服务器，例如HTTP、FTP、LDAP）或者辅助服务器（例如DNS）收到响应。
　　	 *								        注意：某些代理服务器在DNS查询超时时会返回400或者500错误
	 */
	SC_GATEWAY_TIMEOUT("504", "error", "网关访问超时"),
	/**
	 * 异常：HTTP Status 505（HTTP 版本不受支持）    -> 服务器不支持请求的HTTP协议的版本，无法完成处理。这暗示着服务器不能或不愿使用与客户端相同的版本。响应中应当包含一个描述了为何版本不被支持以及服务器支持哪些协议的实体。
	 */
	SC_HTTP_VERSION_NOT_SUPPORTED("505", "error", "HTTP 版本不受支持"),
	/**
	 * 异常：HTTP Status 506（服务器内部配置错误）->  由《透明内容协商协议》（RFC 2295）扩展，代表服务器存在内部配置错误：被请求的协商变元资源被配置为在透明内容协商中使用自己，因此在一个协商处理中不是一个合适的重点。
	 */
	SC_VARIANT_ALSO_NEGOTIATES("506", "error", "服务器内部配置错误" ),
	/**
	 * 异常：HTTP Status 507（服务器无法存储完成请求所必须的内容）-> 这个状况被认为是临时的。WebDAV (RFC 4918)
	 */
	SC_INSUFFICIENT_STORAGE("507", "error", "服务器无法存储完成请求所必须的内容"),
	/**
	 * 异常：HTTP Status 508（存储空间不足）
	 */
	SC_LOOP_DETECTED("508", "error", "服务器存储空间不足"),
	/**
	 * 异常：HTTP Status 509（服务器达到带宽限制）->这不是一个官方的状态码，但是仍被广泛使用。
	 */
	SC_BANDWIDTH_LIMIT_EXCEEDED("509", "error", "服务器达到带宽限制"),
	/**
	 * 异常：HTTP Status 510（获取资源所需要的策略并没有没满足）
	 */
	SC_NOT_EXTENDED("510", "error", "获取资源所需要的策略并没有没满足"),
	/**
	 * 异常：HTTP Status 511（要求网络认证）
	 */
	SC_NETWORK_AUTHENTICATION_REQUIRED("511", "error", "要求网络认证"),

	// --- Custom Server Error ---
	
	SC_SUCCESS("0", "success", "请求成功"),
	
	SC_FAIL("10110","fail", "请求失败"),
	
	SC_EMPTY("10111","fail", "数据为空" ),

	/**
	 * TypeMismatchException	400 (Bad Request)
	 */
	SC_TYPE_MISMATCH("10112", "error", "参数类型不匹配"),
	/**
	 * MissingMatrixVariableException	400 (Bad Request)
	 */
	SC_MISSING_MATRIX_VARIABLE("10113", "error", "缺少矩阵变量"),
	/**
	 * MissingPathVariableException	400 (Bad Request)
	 */
	SC_MISSING_PATH_VARIABLE("10114", "error", "缺少URI模板变量"),
	/**
	 * MissingRequestCookieException	400 (Bad Request)
	 */
	SC_MISSING_REQUEST_COOKIE("10115", "error", "缺少Cookie变量"),
	/**
	 * MissingRequestHeaderException	400 (Bad Request)
	 */
	SC_MISSING_REQUEST_HEADER("10116", "error", "缺少请求头"),
	/**
	 * MissingServletRequestParameterException	400 (Bad Request)
	 */
	SC_MISSING_REQUEST_PARAM("10117", "error", "缺少参数"),
	/**
	 * MissingServletRequestPartException	400 (Bad Request)
	 */
	SC_MISSING_REQUEST_PART("10118", "error", "缺少请求对象"),
	/**
	 * UnsatisfiedServletRequestParameterException	400 (Bad Request)
	 */
	SC_UNSATISFIED_PARAM("10119", "error", "参数规则不满足"),
	/**
	 * ServletRequestBindingException	400 (Bad Request)
	 */
	SC_BINDING_ERROR("10120", "error", "参数绑定错误"),
	/**
	 * JsonProcessingException	400 (Bad Request)
	 * HttpMessageNotReadableException	400 (Bad Request)
	 */
	SC_PARSING_ERROR("10121", "error", "参数解析错误"),
	/**
	 * MethodArgumentNotValidException	400 (Bad Request)
	 * BindException					400 (Bad Request)								      
	 */
	SC_METHOD_ARGUMENT_NOT_VALID("10122", "error", "参数验证失败"),
	
	
	/**
	 * RuntimeException		500 (Internal Server Error)							      
	 */
	SC_RUNTIME_EXCEPTION("10201", "error", "服务器:运行时异常"),
	/**
	 * NullPointerException		500 (Internal Server Error)					      
	 */
	SC_NULL_POINTER_EXCEPTION("10202", "error", "服务器:空值异常"),
	/**
	 * ClassCastException		500 (Internal Server Error)						      
	 */
	SC_CLASS_CAST_EXCEPTION("10203", "error", "服务器:数据类型转换异常"),
	/**
	 * IOException		500 (Internal Server Error)								      
	 */
	SC_IO_EXCEPTION ("10204", "error", "服务器:IO异常"),
	/**
	 * NoSuchMethodException		500 (Internal Server Error)					      
	 */
	SC_NO_SUCH_METHOD_EXCEPTION("10205", "error", "服务器:未知方法异常"),
	/**
	 * IllegalArgumentException		500 (Internal Server Error)					      
	 */
	SC_ILLEGAL_ARGUMENT_EXCEPTION("10206", "error", "服务器:非法参数异常"),
	/**
	 * IndexOutOfBoundsException		500 (Internal Server Error)				      
	 */
	SC_INDEX_OUT_OF_BOUNDS_EXCEPTION("10207", "error", "服务器:数组越界异常"),
	/**
	 * NetworkException		500 (Internal Server Error)							      
	 */
	SC_NETWORK_EXCEPTION("10208", "error", "服务器:网络异常");
