...
/**
 * http://tools.jb51.net/table/http_status_code <br/>
 * http://blog.sina.com.cn/s/blog_61c07ac50102volc.html <br/>
...
public enum HttpExceptions {
	 ...
	 */
	SC_BAD_REQUEST(HttpStatus.BAD_REQUEST, "错误请求"),
	/**
	 * HTTP Status 401（未授权）     -> 请求要求身份验证。 对于需要登录的网页，服务器可能返回此响应
	 */
	SC_UNAUTHORIZED(HttpStatus.UNAUTHORIZED, "请求要求身份验证"),
	/**
	 * HTTP Status 402（ 	保留，将来使用）     ->
	 */
	SC_PAYMENT_REQUIRED(HttpStatus.PAYMENT_REQUIRED, "保留状态码"),
	/**
	 * HTTP Status 403（禁止）    -> 服务器理解请求客户端的请求，但是拒绝执行此请求。
	 */
	SC_FORBIDDEN(HttpStatus.FORBIDDEN, "服务器拒绝请求"),
	/**
	 * 异常：HTTP Status 404（未找到）     -> 服务器找不到请求的资源（网页）。通过此代码，网站设计人员可设置"您所请求的资源无法找到"的个性页面
	 * NoSuchRequestHandlingMethodException    404 (Not Found) 
	 */
	SC_NOT_FOUND(HttpStatus.NOT_FOUND, "服务器找不到请求的资源（网页）"),
	/**
	 * 异常：HTTP Status 405（方法禁用）    ->禁用请求中指定的方法。
	 * HttpRequestMethodNotSupportedException  405 (Method Not Allowed)
	 */
	SC_METHOD_NOT_ALLOWED(HttpStatus.METHOD_NOT_ALLOWED, "客户端请求中的方法被禁止"),
	/**
	 * 异常：HTTP Status 406（不接受）     ->服务器无法根据客户端请求的内容特性完成请求。
	 * HttpMediaTypeNotAcceptableException     406 (Not Acceptable)
	 */
	SC_NOT_ACCEPTABLE(HttpStatus.NOT_ACCEPTABLE, "服务器无法根据客户端请求的内容特性完成请求"),
	/**
	 * 异常：HTTP Status 407（需要代理授权）     ->此状态代码与 401（未授权）类似，但指定请求者应当授权使用代理。
	 */
	SC_PROXY_AUTHENTICATION_REQUIRED(HttpStatus.PROXY_AUTHENTICATION_REQUIRED, "要求进行代理身份验证"),
	/**
	 * 异常：HTTP Status 408（请求超时）    ->服务器等待客户端发送的请求时间过长，超时。
	 */
	SC_REQUEST_TIMEOUT(HttpStatus.REQUEST_TIMEOUT, "服务器等候请求时发生超时"),
	/**
	 * 异常：HTTP Status 409（冲突）     ->服务器在完成请求时发生冲突。 服务器必须在响应中包含有关冲突的信息。
	 */
	SC_CONFLICT(HttpStatus.CONFLICT, "服务器找不到请求的地址"),
	/**
	 * 异常：HTTP Status 410（已删除）    -> 如果请求的资源已永久删除，服务器就会返回此响应。
	 */
	SC_GONE(HttpStatus.GONE, "服务器找不到请求的地址" ),
	/**
	 * 异常：HTTP Status 411（需要有效长度）     ->服务器无法处理客户端发送的不带Content-Length的请求信息。
	 */
	SC_LENGTH_REQUIRED(HttpStatus.LENGTH_REQUIRED, "服务器拒绝接受不带Content-Length请求头的客户端请求"),
	/**
	 * 异常：HTTP Status 412（未满足前提条件）     ->服务器未满足请求者在请求中设置的其中一个前提条件。
	 */
	SC_PRECONDITION_FAILED(HttpStatus.PRECONDITION_FAILED, "客户端请求信息的先决条件错误"),
	/**
	 * 异常：HTTP Status 413（请求实体过大）     ->服务器无法处理请求，因为请求实体过大，超出服务器的处理能力。为防止客户端的连续请求，服务器可能会关闭连接。如果只是服务器暂时无法处理，则会包含一个Retry-After的响应信息。
	 */
	SC_REQUEST_TOO_LONG(HttpStatus.URI_TOO_LONG, "服务器无法处理请求，因为请求实体过大，超出服务器的处理能力" ),
	/**
	 * 异常：HTTP Status 414（请求的 URI 过长） 请求的 URI（通常为网址）过长，服务器无法处理。
	 */
	@SuppressWarnings("deprecation")
	SC_REQUEST_URI_TOO_LONG(HttpStatus.REQUEST_URI_TOO_LONG, "请求的URI过长（URI通常为网址），服务器无法处理"),
	/**
	 * 异常：HTTP Status 415（不支持的媒体类型）    ->请求的格式不受请求页面的支持。
	 * HttpMediaTypeNotSupportedException      415 (Unsupported Media Type)
	 */
	SC_UNSUPPORTED_MEDIA_TYPE(HttpStatus.UNSUPPORTED_MEDIA_TYPE, "服务器无法处理请求附带的媒体格式"),
	/**
	 * 异常：HTTP Status 416（请求范围不符合要求）     ->如果页面无法提供请求的范围，则服务器会返回此状态代码。
	 */
	SC_REQUESTED_RANGE_NOT_SATISFIABLE(HttpStatus.REQUESTED_RANGE_NOT_SATISFIABLE, "客户端请求的范围无效"),
	/**
	 * 异常：HTTP Status 417（未满足期望值）     ->服务器未满足”Expect”请求标头字段的要求。
	 */
	SC_EXPECTATION_FAILED(HttpStatus.EXPECTATION_FAILED, "服务器无法满足Expect的请求头信息"),
	/**
	 * 异常：HTTP Status 422（无法处理的请求实体） ->请求格式正确，但是由于含有语义错误，无法响应。
	 */
	SC_UNPROCESSABLE_ENTITY(HttpStatus.UNPROCESSABLE_ENTITY, "无法处理的请求实体"),
	/**
	 * 异常：HTTP Status 423（当前资源被锁定）
	 */
	SC_LOCKED(HttpStatus.LOCKED, "当前资源被锁定 "),
	/**
	 * 异常：HTTP Status 424（依赖导致的失败）->由于之前的某个请求发生的错误，导致当前请求失败，例如 PROPPATCH。
	 */
	SC_FAILED_DEPENDENCY(HttpStatus.FAILED_DEPENDENCY, "依赖导致的失败"),
	/**
	 * 异常：HTTP Status 424（客户端应当切换到TLS/1.0）
	 */
	SC_UPGRADE_REQUIRED(HttpStatus.UPGRADE_REQUIRED, "客户端应当切换到TLS/1.0"),
	/**
	 * 异常：HTTP Status 428（要求先决条件）    -> 先决条件是客户端发送 HTTP 请求时，如果想要请求能成功必须满足一些预设的条件。
	 */
	SC_PRECONDITION_REQUIRED(HttpStatus.PRECONDITION_REQUIRED, "要求先决条件"),
	/**
	 * 异常：HTTP Status 429（太多请求）    -> 当你需要限制客户端请求某个服务数量时，该状态码就很有用，也就是请求速度限制。
	 */
	SC_TOO_MANY_REQUESTS(HttpStatus.TOO_MANY_REQUESTS, "太多请求"),
	/**
	 * 异常：HTTP Status 431（请求头字段太大）    -> 某些情况下，客户端发送 HTTP 请求头会变得很大，那么服务器可发送 431 Request Header Fields Too Large 来指明该问题。
	 */
	SC_REQUEST_HEADER_FIELDS_TOO_LARGE(HttpStatus.REQUEST_HEADER_FIELDS_TOO_LARGE, "请求头字段太大" ),
	/**
	 * 异常：HTTP Status 451（因法律原因不可用）    ->
	 */
	SC_UNAVAILABLE_FOR_LEGAL_REASONS(HttpStatus.UNAVAILABLE_FOR_LEGAL_REASONS, "该请求因法律原因不可用"),
	 ...
	 */
	SC_INTERNAL_SERVER_ERROR(HttpStatus.INTERNAL_SERVER_ERROR, "服务器内部错误，无法完成请求"),
	/**
	 * 异常：HTTP Status 501（尚未实施）     ->服务器不具备完成请求的功能。 例如，服务器无法识别请求方法时可能会返回此代码。
	 */
	SC_NOT_IMPLEMENTED(HttpStatus.NOT_IMPLEMENTED, "服务器不支持请求的功能，无法完成请求"),
	/**
	 * 异常：HTTP Status 502（错误网关）     -> 作为网关或者代理工作的服务器尝试执行请求时，从上游服务器接收到无效的响应。
	 */
	SC_BAD_GATEWAY(HttpStatus.BAD_GATEWAY, "错误网关"),
	/**
	 ...
	 */
	SC_SERVICE_UNAVAILABLE(HttpStatus.SERVICE_UNAVAILABLE, "服务器目前无法使用（由于超载或停机维护）"),
	/**
	 * 异常：HTTP Status 504（网关访问超时）     -> 作为网关或者代理工作的服务器尝试执行请求时，未能及时从上游服务器（URI标识出的服务器，例如HTTP、FTP、LDAP）或者辅助服务器（例如DNS）收到响应。
　　	 *								        注意：某些代理服务器在DNS查询超时时会返回400或者500错误
	 */
	SC_GATEWAY_TIMEOUT(HttpStatus.GATEWAY_TIMEOUT, "网关访问超时"),
	/**
	 * 异常：HTTP Status 505（HTTP 版本不受支持）    -> 服务器不支持请求的HTTP协议的版本，无法完成处理。这暗示着服务器不能或不愿使用与客户端相同的版本。响应中应当包含一个描述了为何版本不被支持以及服务器支持哪些协议的实体。
	 */
	SC_HTTP_VERSION_NOT_SUPPORTED(HttpStatus.HTTP_VERSION_NOT_SUPPORTED, "HTTP 版本不受支持"),
	/**
	 * 异常：HTTP Status 506（服务器内部配置错误）->  由《透明内容协商协议》（RFC 2295）扩展，代表服务器存在内部配置错误：被请求的协商变元资源被配置为在透明内容协商中使用自己，因此在一个协商处理中不是一个合适的重点。
	 */
	SC_VARIANT_ALSO_NEGOTIATES(HttpStatus.VARIANT_ALSO_NEGOTIATES, "服务器内部配置错误" ),
	/**
	 * 异常：HTTP Status 507（服务器无法存储完成请求所必须的内容）-> 这个状况被认为是临时的。WebDAV (RFC 4918)
	 */
	SC_INSUFFICIENT_STORAGE(HttpStatus.INSUFFICIENT_STORAGE, "服务器无法存储完成请求所必须的内容"),
	/**
	 * 异常：HTTP Status 508（存储空间不足）
	 */
	SC_LOOP_DETECTED(HttpStatus.LOOP_DETECTED, "服务器存储空间不足"),
	/**
	 * 异常：HTTP Status 509（服务器达到带宽限制）->这不是一个官方的状态码，但是仍被广泛使用。
	 */
	SC_BANDWIDTH_LIMIT_EXCEEDED(HttpStatus.BANDWIDTH_LIMIT_EXCEEDED, "服务器达到带宽限制"),
	/**
	 * 异常：HTTP Status 510（获取资源所需要的策略并没有没满足）
	 */
	SC_NOT_EXTENDED(HttpStatus.NOT_EXTENDED, "获取资源所需要的策略并没有没满足"),
	/**
	 * 异常：HTTP Status 511（要求网络认证）
	 */
	SC_NETWORK_AUTHENTICATION_REQUIRED(HttpStatus.NETWORK_AUTHENTICATION_REQUIRED, "要求网络认证");
	...
	HttpExceptions(HttpStatus httpStatus, String reasonPhrase) {
